...
public class BinaryHeap<T> implements PQ<T> {
	...
	/**Return the root element in the heap which would be the minimum
	 * or maximum element depending upon the comparator
	 */
	...
	public void add(T x) {
		...
		//Resize the heap if it has reached its limit
		if(size == pq.length - 1) {
			T[] temp = pq;
	...
	//Remove the minimum or maximum element based on the type of heap
	public T remove() {
		T val = peek();
	...
	//Return the minimum or maximum element based on the type of heap
	public T peek() {
		if(size == 0) {
